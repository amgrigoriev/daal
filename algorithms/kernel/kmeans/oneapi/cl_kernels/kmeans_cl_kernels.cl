...
DECLARE_SOURCE(
    ...
    __kernel void reduce_assignments(__global const algorithmFPType * centroidsSq, __global const algorithmFPType * distances, __global int * assignments, __global algorithmFPType * mindistances,
                                     int N, int K) {
        ...
        {
            algorithmFPType curVal   = (i == numgrp - 1 && local_id >= rem) ? HUGE : (distances[global_id + N * (local_id + i * size)] + centroidsSq[local_id + i * size]);
        ...
        int curInd = 1;
        if (minIdx < numgrp - 1 || local_id < rem) curInd = (distances[global_id + N * (local_id + minIdx * size)] + centroidsSq[local_id + minIdx * size] > minVal) ? 1 : -local_id;
    ...
    __kernel void count_empty_clusters(__global const int * partialCentroidsCounters, int K, int parts,
                                         __global int * numEmptyClusters) 
    {
        const int local_id   = get_local_id(1);
